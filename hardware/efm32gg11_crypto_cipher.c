...
#include "aead/gcm.h"
#include "hardware/efm32gg11_crypto.h"
#include "hardware/efm32gg11_crypto_cipher.h"
#include "debug.h"
   ...
   uint8_t *c, size_t length, uint8_t *t, size_t tLen)
{
   ...
   osMemset(b, 0, 16);
   CRYPTO_DataWrite(&CRYPTO0->DDATA3, context->m[0]);
   CRYPTO_DataWrite(&CRYPTO0->DDATA3, (const uint32_t *) b);

   //Swap the bits of H and store the resulting value in DDATA3
   ...
   CRYPTO_KeyBufWrite(CRYPTO0, aesContext->ek, keyWidth);
   ...
   CRYPTO_DataWrite(&CRYPTO0->DATA0, (const uint32_t *) j);
   CRYPTO_EXECUTE_1(CRYPTO0, CRYPTO_CMD_INSTR_AESENC);
   CRYPTO_InstructionSequenceWait(CRYPTO0);
   CRYPTO_DataRead(&CRYPTO0->DATA0, (uint32_t *) b);
   osMemcpy(t, b, tLen);
...
void gcmMul(GcmContext *context, uint8_t *x)
{
   ...
   CRYPTO_DataWrite(&CRYPTO0->DDATA1, (const uint32_t *) x);
   CRYPTO_DataWrite(&CRYPTO0->DDATA1, pad);

   //Perform GF(2^128) multiplication
   CRYPTO_EXECUTE_8(CRYPTO0,
