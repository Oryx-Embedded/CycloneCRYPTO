/**
 * @file pic32cx_bz2_crypto_cipher.c
...
#if (PIC32CX_BZ2_CRYPTO_CIPHER_SUPPORT == ENABLED && AES_SUPPORT == ENABLED)
   ...
   uint8_t *output, size_t length, uint32_t mode)
{
   ...
   osAcquireMutex(&pic32cxbz2CryptoMutex);
   ...
   osReleaseMutex(&pic32cxbz2CryptoMutex);
}
