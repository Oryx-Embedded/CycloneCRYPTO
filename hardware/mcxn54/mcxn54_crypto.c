/**
 * @file mcxn54_crypto.c
 * @brief NXP MCX N54 hardware cryptographic accelerator
 *
 * @section License
 *
...
#include "hardware/mcxn54/mcxn54_crypto_trng.h"
#include "debug.h"
...
error_t mcxn54CryptoInit(void)
{
   ...
   if(!osCreateMutex(&mcxn54CryptoMutex))
   {
...
#if (MCXN54_CRYPTO_TRNG_SUPPORT == ENABLED)
   //Check status code
   if(!error)
   {
