/**
 * @file m5531_crypto_cipher.c
 * @brief M5531 cipher hardware accelerator
 *
 * @section License
 *
...
#include "hardware/m5531/m5531_crypto.h"
#include "hardware/m5531/m5531_crypto_cipher.h"
...
#if (M5531_CRYPTO_CIPHER_SUPPORT == ENABLED)
...
void aesLoadKey(AesContext *context)
{
   ...
   //Read mode register
   temp = CRYPTO->AES_CTL & ~CRYPTO_AES_CTL_KEYSZ_Pos;

   //Check the length of the key
   if(context->nr == 10)
   {
      //10 rounds are required for 128-bit key
      CRYPTO->AES_CTL = temp | CRYPTO_AES_CTL_KEYSZ_128B;

      //Set the 128-bit encryption key
      CRYPTO->AES_KEY[0] = context->ek[0];
      CRYPTO->AES_KEY[1] = context->ek[1];
      CRYPTO->AES_KEY[2] = context->ek[2];
      CRYPTO->AES_KEY[3] = context->ek[3];
   }
   else if(context->nr == 12)
   {
      //12 rounds are required for 192-bit key
      CRYPTO->AES_CTL = temp | CRYPTO_AES_CTL_KEYSZ_192B;
      ...
      CRYPTO->AES_KEY[2] = context->ek[2];
      CRYPTO->AES_KEY[3] = context->ek[3];
      CRYPTO->AES_KEY[4] = context->ek[4];
      CRYPTO->AES_KEY[5] = context->ek[5];
   }
   else
   {
      //14 rounds are required for 256-bit key
      CRYPTO->AES_CTL = temp | CRYPTO_AES_CTL_KEYSZ_256B;
      ...
      CRYPTO->AES_KEY[4] = context->ek[4];
      CRYPTO->AES_KEY[5] = context->ek[5];
      CRYPTO->AES_KEY[6] = context->ek[6];
      CRYPTO->AES_KEY[7] = context->ek[7];
   }
}
...
void aesProcessDataBlock(const uint8_t *input, uint8_t *output)
{
   ...
   CRYPTO->AES_CTL |= CRYPTO_AES_CTL_DMALAST_Msk | CRYPTO_AES_CTL_START_Msk;

   //Poll INBUFFULL flag
   while((CRYPTO->AES_STS & CRYPTO_AES_STS_INBUFFULL_Msk) != 0)
   {
   }

   //Write input block CRYPTO_AES_DATIN
   CRYPTO->AES_DATIN = __UNALIGNED_UINT32_READ(input);
   CRYPTO->AES_DATIN = __UNALIGNED_UINT32_READ(input + 4);
   CRYPTO->AES_DATIN = __UNALIGNED_UINT32_READ(input + 8);
   CRYPTO->AES_DATIN = __UNALIGNED_UINT32_READ(input + 12);

   //Poll OUTBUFEMPTY flag
   while((CRYPTO->AES_STS & CRYPTO_AES_STS_OUTBUFEMPTY_Msk) != 0)
   {
   }

   //Read output block from CRYPTO_AES_DATOUT
   temp = CRYPTO->AES_DATOUT;
   ...
   CRYPTO->AES_IV[0] = CRYPTO->AES_FDBCK[0];
   CRYPTO->AES_IV[1] = CRYPTO->AES_FDBCK[1];
   CRYPTO->AES_IV[2] = CRYPTO->AES_FDBCK[2];
   CRYPTO->AES_IV[3] = CRYPTO->AES_FDBCK[3];
}
   ...
   uint8_t *output, size_t length, uint32_t opmode)
{
   ...
   osAcquireMutex(&m5531CryptoMutex);

   //Reset CRYPTO controller
   SYS_ResetModule(SYS_CRYPTO0RST);

   //Configure operation mode
   CRYPTO->AES_CTL = CRYPTO_AES_CTL_KEYPRT_Msk | CRYPTO_AES_CTL_INSWAP_Msk |
      CRYPTO_AES_CTL_OUTSWAP_Msk | opmode;
   ...
   if(iv != NULL)
   {
      ...
      CRYPTO->AES_IV[0] = LOAD32BE(iv);
      CRYPTO->AES_IV[1] = LOAD32BE(iv + 4);
      CRYPTO->AES_IV[2] = LOAD32BE(iv + 8);
      CRYPTO->AES_IV[3] = LOAD32BE(iv + 12);
   }
   ...
   if(iv != NULL)
   {
      ...
      temp = CRYPTO->AES_FDBCK[0];
      STORE32BE(temp, iv);
      temp = CRYPTO->AES_FDBCK[1];
      STORE32BE(temp, iv + 4);
      temp = CRYPTO->AES_FDBCK[2];
      STORE32BE(temp, iv + 8);
      temp = CRYPTO->AES_FDBCK[3];
...
void aesEncryptBlock(AesContext *context, const uint8_t *input, uint8_t *output)
{
   ...
   aesProcessData(context, NULL, input, output, AES_BLOCK_SIZE,
      CRYPTO_AES_CTL_OPMODE_ECB | CRYPTO_AES_CTL_ENCRYPTO_Msk);
}
   ...
   uint8_t *output, size_t length, uint32_t opmode)
{
   ...
   //Configure operation mode
   CRYPTO->AES_CTL = CRYPTO_AES_CTL_KEYPRT_Msk | CRYPTO_AES_CTL_INSWAP_Msk |
      CRYPTO_AES_CTL_OUTSWAP_Msk | CRYPTO_AES_CTL_SM4EN_Msk | opmode;

   //Set encryption key
   CRYPTO->AES_KEY[0] = context->rk[0];
   CRYPTO->AES_KEY[1] = context->rk[1];
   CRYPTO->AES_KEY[2] = context->rk[2];
   CRYPTO->AES_KEY[3] = context->rk[3];
   ...
   const uint8_t *p, uint8_t *c, size_t length)
{
   ...
   if(cipher == AES_CIPHER_ALGO)
   {
      ...
      else if((length % AES_BLOCK_SIZE) == 0)
      {
         ...
         aesProcessData(context, NULL, p, c, length, CRYPTO_AES_CTL_OPMODE_ECB |
            CRYPTO_AES_CTL_ENCRYPTO_Msk);
      }
      else
      {
   ...
   if(cipher == SM4_CIPHER_ALGO)
   {
      ...
      {
         //Encrypt payload data
         sm4ProcessData(context, NULL, p, c, length, CRYPTO_AES_CTL_OPMODE_ECB |
            CRYPTO_AES_CTL_ENCRYPTO_Msk);
      }
      else
      {
   ...
   const uint8_t *c, uint8_t *p, size_t length)
{
   ...
   if(cipher == AES_CIPHER_ALGO)
   {
      ...
      else if((length % AES_BLOCK_SIZE) == 0)
      {
         ...
         aesProcessData(context, NULL, c, p, length, CRYPTO_AES_CTL_OPMODE_ECB);
      }
      else
      {
         //The length of the payload must be a multiple of the block size
   ...
   if(cipher == SM4_CIPHER_ALGO)
   {
      ...
      {
         //Decrypt payload data
         sm4ProcessData(context, NULL, c, p, length, CRYPTO_AES_CTL_OPMODE_ECB);
      }
      else
      {
         //The length of the payload must be a multiple of the block size
   ...
   uint8_t *iv, const uint8_t *p, uint8_t *c, size_t length)
{
   ...
   if(cipher == AES_CIPHER_ALGO)
   {
      ...
      {
         //Encrypt payload data
         aesProcessData(context, iv, p, c, length, CRYPTO_AES_CTL_OPMODE_CBC |
            CRYPTO_AES_CTL_ENCRYPTO_Msk);
      }
      else
      {
   ...
   if(cipher == SM4_CIPHER_ALGO)
   {
      ...
      {
         //Encrypt payload data
         sm4ProcessData(context, iv, p, c, length, CRYPTO_AES_CTL_OPMODE_CBC |
            CRYPTO_AES_CTL_ENCRYPTO_Msk);
      }
      else
      {
   ...
   uint8_t *iv, const uint8_t *c, uint8_t *p, size_t length)
{
   ...
   if(cipher == AES_CIPHER_ALGO)
   {
      ...
      {
         //Decrypt payload data
         aesProcessData(context, iv, c, p, length, CRYPTO_AES_CTL_OPMODE_CBC);
      }
      else
      {
         //The length of the payload must be a multiple of the block size
   ...
   if(cipher == SM4_CIPHER_ALGO)
   {
      ...
      {
         //Decrypt payload data
         sm4ProcessData(context, iv, c, p, length, CRYPTO_AES_CTL_OPMODE_CBC);
      }
      else
      {
         //The length of the payload must be a multiple of the block size
   ...
   uint8_t *iv, const uint8_t *p, uint8_t *c, size_t length)
{
   ...
   if(cipher == AES_CIPHER_ALGO)
   {
      ...
      if(s == (AES_BLOCK_SIZE * 8))
      {
         ...
         {
            //Encrypt payload data
            aesProcessData(context, iv, p, c, length, CRYPTO_AES_CTL_OPMODE_CFB |
               CRYPTO_AES_CTL_ENCRYPTO_Msk);
         }
         else
         {
   ...
   if(cipher == SM4_CIPHER_ALGO)
   {
      ...
      if(s == (SM4_BLOCK_SIZE * 8))
      {
         ...
         {
            //Encrypt payload data
            sm4ProcessData(context, iv, p, c, length, CRYPTO_AES_CTL_OPMODE_CFB |
               CRYPTO_AES_CTL_ENCRYPTO_Msk);
         }
         else
         {
   ...
   uint8_t *iv, const uint8_t *c, uint8_t *p, size_t length)
{
   ...
   if(cipher == AES_CIPHER_ALGO)
   {
      ...
      if(s == (AES_BLOCK_SIZE * 8))
      {
         ...
         {
            //Decrypt payload data
            aesProcessData(context, iv, c, p, length, CRYPTO_AES_CTL_OPMODE_CFB);
         }
         else
         {
            //No data to process
   ...
   if(cipher == SM4_CIPHER_ALGO)
   {
      ...
      if(s == (SM4_BLOCK_SIZE * 8))
      {
         ...
         {
            //Decrypt payload data
            sm4ProcessData(context, iv, c, p, length, CRYPTO_AES_CTL_OPMODE_CFB);
         }
         else
         {
            //No data to process
   ...
   uint8_t *iv, const uint8_t *p, uint8_t *c, size_t length)
{
   ...
   if(cipher == AES_CIPHER_ALGO)
   {
      ...
      if(s == (AES_BLOCK_SIZE * 8))
      {
         ...
         {
            //Encrypt payload data
            aesProcessData(context, iv, p, c, length, CRYPTO_AES_CTL_OPMODE_OFB |
               CRYPTO_AES_CTL_ENCRYPTO_Msk);
         }
         else
         {
   ...
   if(cipher == SM4_CIPHER_ALGO)
   {
      ...
      if(s == (SM4_BLOCK_SIZE * 8))
      {
         ...
         {
            //Encrypt payload data
            sm4ProcessData(context, iv, p, c, length, CRYPTO_AES_CTL_OPMODE_OFB |
               CRYPTO_AES_CTL_ENCRYPTO_Msk);
         }
         else
         {
   ...
   if((m % 8) == 0 && m <= (cipher->blockSize * 8))
   {
      ...
      if(cipher == AES_CIPHER_ALGO)
      {
         ...
         while(length > 0)
         {
            ...
            //Encrypt payload data
            aesProcessData(context, iv, p, c, n, CRYPTO_AES_CTL_OPMODE_CTR |
               CRYPTO_AES_CTL_ENCRYPTO_Msk);
      ...
      if(cipher == SM4_CIPHER_ALGO)
      {
         ...
         while(length > 0)
         {
            ...
            //Encrypt payload data
            sm4ProcessData(context, iv, p, c, n, CRYPTO_AES_CTL_OPMODE_CTR |
               CRYPTO_AES_CTL_ENCRYPTO_Msk);
   ...
   if(context->cipherAlgo == AES_CIPHER_ALGO)
   {
      ...
      for(i = 0; i < length; i += n)
      {
         ...
         aesProcessData(context->cipherContext, iv, p + i, c + i, n,
            CRYPTO_AES_CTL_OPMODE_CTR | CRYPTO_AES_CTL_ENCRYPTO_Msk);
