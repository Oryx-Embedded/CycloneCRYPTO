/**
 * @file m5531_crypto_pkc.c
 * @brief M5531 public-key hardware accelerator
 *
 * @section License
 *
...
#include "hardware/m5531/m5531_crypto_pkc.h"
#include "pkc/rsa.h"
#include "ecc/ec.h"
...
#if (M5531_CRYPTO_PKC_SUPPORT == ENABLED)
...
#pragma location = M5531_CRYPTO_RAM_SECTION
static M5531RsaArgs rsaArgs;
...
//RSA primitive arguments
static M5531RsaArgs rsaArgs
   __attribute__((aligned(4), __section__(M5531_CRYPTO_RAM_SECTION)));
...
error_t mpiExpModFast(Mpi *r, const Mpi *a, const Mpi *e, const Mpi *p)
{
      ...
      (aLen <= 512 && eLen <= 512 && pLen == 512))
   {
      ...
      CRYPTO->RSA_SADDR[0] = (uint32_t) rsaArgs.m;
      CRYPTO->RSA_SADDR[1] = (uint32_t) rsaArgs.n;
      CRYPTO->RSA_SADDR[2] = (uint32_t) rsaArgs.e;

      //Program DMA destination address to register CRYPTO_RSA_DADDR
      CRYPTO->RSA_DADDR = (uint32_t) rsaArgs.r;

      //Select appropriate key length
      CRYPTO->RSA_CTL = (((pLen / 128) - 1) << CRYPTO_RSA_CTL_KEYLENG_Pos);
   ...
   const EcPoint3 *s)
{
   ...
   if(modLen <= 576 && orderLen <= 576)
   {
      ...
      eccImportScalar(CRYPTO->ECC_N, modLen, curve->p);
      eccImportScalar(CRYPTO->ECC_A, modLen, curve->a);
      eccImportScalar(CRYPTO->ECC_B, modLen, curve->b);
      eccImportScalar(CRYPTO->ECC_K, orderLen, d);
      eccImportScalar(CRYPTO->ECC_X1, modLen, s->x);
      eccImportScalar(CRYPTO->ECC_Y1, modLen, s->y);
      eccImportScalar(CRYPTO->ECC_X2, orderLen, curve->q);

      //Set up a point multiplication operation
      CRYPTO->ECC_CTL = (modLen << CRYPTO_ECC_CTL_CURVEM_Pos) |
         CRYPTO_ECC_CTL_SCAP_Msk | (0 << CRYPTO_ECC_CTL_ECCOP_Pos) |
         CRYPTO_ECC_CTL_FSEL_Msk;

      //Clear ECC interrupt flag
      CRYPTO->INTSTS = CRYPTO_INTSTS_ECCIF_Msk;
      //Start operation
      CRYPTO->ECC_CTL |= CRYPTO_ECC_CTL_START_Msk;

      //Wait for the operation to complete
      while((CRYPTO->INTSTS & CRYPTO_INTSTS_ECCIF_Msk) == 0)
      {
      }

      //Copy the x-coordinate of the result
      ecScalarSetInt(r->x, 0, EC_MAX_MODULUS_SIZE);
      eccExportScalar(CRYPTO->ECC_X1, modLen, r->x);

      //Copy the y-coordinate of the result
      ecScalarSetInt(r->y, 0, EC_MAX_MODULUS_SIZE);
      eccExportScalar(CRYPTO->ECC_Y1, modLen, r->y);
