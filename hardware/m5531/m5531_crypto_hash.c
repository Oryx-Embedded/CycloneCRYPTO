/**
 * @file m5531_crypto_hash.c
 * @brief M5531 hash hardware accelerator
 *
 * @section License
 *
...
#include "hardware/m5531/m5531_crypto_hash.h"
#include "hash/hash_algorithms.h"
#include "xof/keccak.h"
#include "debug.h"

//Check crypto library configuration
#if (M5531_CRYPTO_HASH_SUPPORT == ENABLED)
   ...
   size_t length, uint32_t *h, size_t hLen)
{
   ...
   CRYPTO->HMAC_CTL = CRYPTO_HMAC_CTL_INSWAP_Msk | CRYPTO_HMAC_CTL_OUTSWAP_Msk |
      CRYPTO_HMAC_CTL_DMACSCAD_Msk | opmode;

   //SHA-1, SHA-224, SHA-256 or SM3 algorithm?
   if(opmode == CRYPTO_HMAC_CTL_OPMODE_SHA1 || opmode == CRYPTO_HMAC_CTL_OPMODE_SHA224 ||
      opmode == CRYPTO_HMAC_CTL_OPMODE_SHA256 || opmode == CRYPTO_HMAC_CTL_SM3EN_Msk)
   {
      //Restore initial hash value
      for(i = 0; i < hLen; i++)
      {
         CRYPTO->HMAC_FDBCK[2 * i] = h[i];
      }
   }
   else
   {
   ...
   for(i = 0; i < length; i += 4)
   {
      ...
      while((CRYPTO->HMAC_STS & CRYPTO_HMAC_STS_DATINREQ_Msk) == 0)
      {
      }

      //Write one word of data
      CRYPTO->HMAC_DATIN = __UNALIGNED_UINT32_READ(data + i);
   }
...
void sha1Update(Sha1Context *context, const void *data, size_t length)
{
   ...
   while(length > 0)
   {
      ...
      if(context->size == 0 && length >= 64)
      {
         ...
         //Update hash value
         hashProcessData(CRYPTO_HMAC_CTL_OPMODE_SHA1, data, n, context->h,
            SHA1_DIGEST_SIZE / 4);
      ...
      else
      {
         ...
         {
            //Update hash value
            hashProcessData(CRYPTO_HMAC_CTL_OPMODE_SHA1, context->buffer,
...
{
   //Update hash value
   hashProcessData(CRYPTO_HMAC_CTL_OPMODE_SHA1, context->buffer, 64,
...
void sha256Update(Sha256Context *context, const void *data, size_t length)
{
   ...
   while(length > 0)
   {
      ...
      if(context->size == 0 && length >= 64)
      {
         ...
         //Update hash value
         hashProcessData(CRYPTO_HMAC_CTL_OPMODE_SHA256, data, n, context->h,
      ...
      else
      {
         ...
         {
            //Update hash value
            hashProcessData(CRYPTO_HMAC_CTL_OPMODE_SHA256, context->buffer,
...
{
   //Update hash value
   hashProcessData(CRYPTO_HMAC_CTL_OPMODE_SHA256, context->buffer, 64,
...
void sha512Update(Sha512Context *context, const void *data, size_t length)
{
   ...
   while(length > 0)
   {
      ...
      if(context->size == 0 && length >= 128)
      {
         ...
         //Update hash value
         hashProcessData(CRYPTO_HMAC_CTL_OPMODE_SHA512, data, n,
      ...
      else
      {
         ...
         {
            //Update hash value
            hashProcessData(CRYPTO_HMAC_CTL_OPMODE_SHA512, context->buffer,
...
{
   //Update hash value
   hashProcessData(CRYPTO_HMAC_CTL_OPMODE_SHA512, context->buffer, 128,
      (uint32_t *) context->h, SHA512_DIGEST_SIZE / 4);
}
...
void sm3Update(Sm3Context *context, const void *data, size_t length)
{
   ...
   while(length > 0)
   {
      ...
      if(context->size == 0 && length >= 64)
      {
         ...
         //Update hash value
         hashProcessData(CRYPTO_HMAC_CTL_SM3EN_Msk, data, n, context->h,
            SM3_DIGEST_SIZE / 4);
      ...
      else
      {
         ...
         {
            //Update hash value
            hashProcessData(CRYPTO_HMAC_CTL_SM3EN_Msk, context->buffer,
...
{
   //Update hash value
   hashProcessData(CRYPTO_HMAC_CTL_SM3EN_Msk, context->buffer, 64,
   ...
   uint32_t *a)
{
   ...
   if(blockSize == 72)
   {
      opmode = CRYPTO_HMAC_CTL_OPMODE_SHA3_512;
   }
   else if(blockSize == 104)
   {
      opmode = CRYPTO_HMAC_CTL_OPMODE_SHA3_384;
   }
   else if(blockSize == 136)
   {
      opmode = CRYPTO_HMAC_CTL_OPMODE_SHA3_256;
   }
   else
   {
      opmode = CRYPTO_HMAC_CTL_OPMODE_SHA3_224;
   }
   ...
   //Select the relevant hash algorithm
   CRYPTO->HMAC_CTL = CRYPTO_HMAC_CTL_INSWAP_Msk | CRYPTO_HMAC_CTL_OUTSWAP_Msk |
      CRYPTO_HMAC_CTL_SHA3EN_Msk | CRYPTO_HMAC_CTL_DMACSCAD_Msk | opmode;

   //Restore state array
   for(i = 0; i < 16; i++)
   {
      CRYPTO->HMAC_FDBCK[i] = reverseInt32(a[49 - i]);
   }

   for(i = 0; i < 34; i++)
   {
      CRYPTO->HMAC_FDBCK[54 + i] = reverseInt32(a[33 - i]);
   }
   ...
   for(i = 0; i < 34; i++)
   {
      a[i] = reverseInt32(CRYPTO->HMAC_FDBCK[87 - i]);
   }

   for(i = 0; i < 16; i++)
   {
      a[34 + i] = reverseInt32(CRYPTO->HMAC_FDBCK[15 - i]);
   }
