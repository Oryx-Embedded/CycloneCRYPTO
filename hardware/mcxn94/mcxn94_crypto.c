/**
 * @file mcxn94_crypto.c
 * @brief NXP MCX N94 hardware cryptographic accelerator
 *
 * @section License
 *
...
#include "hardware/mcxn94/mcxn94_crypto.h"
#include "hardware/mcxn94/mcxn94_crypto_trng.h"
#include "debug.h"
...
error_t mcxn94CryptoInit(void)
{
   ...
   if(!osCreateMutex(&mcxn94CryptoMutex))
   {
...
#if (MCXN94_CRYPTO_TRNG_SUPPORT == ENABLED)
   //Check status code
   if(!error)
   {
